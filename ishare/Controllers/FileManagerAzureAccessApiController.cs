...
namespace Ishare.Controllers
{
    public class FileManagerAzureAccessApiController : Controller
    {
        ...
        async Task<object> CreateDirectory(string directoryName)
        {
        ...
        async Task<object> UploadBlob(string blobName)
        {
            if (blobName.EndsWith("/"))
                return CreateErrorResult("Invalid blob name.");

            CloudBlockBlob blob = Container.GetBlockBlobReference(blobName);
            if (await blob.ExistsAsync() && blob.Properties.Length > MaxBlobSize)
            {
                await blob.DeleteAsync();
                return CreateErrorResult();
            }
